function [level,bw] = threshToolMod(im,message,cmap,defaultLevel) %mainfunction
...
%
% downloaded from MATLAB Central and slightly modified by Jason Keller
% author: Robert Bemis
%
...
%
%   input 'message' is the title of the figure to display
%
...
if nargin>3 %user specified DEFAULTLEVEL
  dataType = class(im);
  switch dataType
...
h_fig = figure;
set(h_fig,'Name',message); %JAK - add title to figure
set(h_fig,'ToolBar','Figure')
%user specified cmap:
if nargin>2 && isstr(cmap) && strmatch(lower(cmap),'gray')
  full_map = gray(num_colors);
elseif nargin>2 && isnumeric(cmap) && length(size(cmap))==2 && size(cmap,2)==3
  full_map = cmap;
else
  full_map = jet(num_colors);  %default cmap
end
...
if nargin>3 %user specified default level
...
% PLACE TEXT LOCATION ON COLORBAR
